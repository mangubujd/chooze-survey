...
})({"2oeXr":[function(require,module,exports) {
var global = arguments[3];
var HMR_HOST = null;
var HMR_PORT = 1234;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "f23b19fb00aa10c2";
"use strict";
...
},{}],"le1MK":[function(require,module,exports) {
document.addEventListener("DOMContentLoaded", function() {
    ...
    const displayStep = ()=>{
        ...
        if (currentStep == 1) backButton.classList.remove("visible");
        else backButton.classList.add("visible");
        // Progress
        progressBar.style.transform = `translateX(-${currentStep / steps.length * -100 + 100}%)`;
        // TRACKING
        const events = {
            event: "click",
    ...
    steps && steps.forEach((step)=>{
        ...
        inputs.forEach((input)=>{
            input.addEventListener("click", (e)=>{
                if (e.target.checked) goToNextStep();
            });
        });
    });
    // Code Input
    ...
    codes && codes.forEach((code, index)=>{
    ...
    phone && phone.addEventListener("change", (e)=>{
    ...
    const formValidationStep = document.getElementById("contact-survey-validation-form");
    const formPreferencesStep = document.getElementById("contact-survey-preferences-form");
    ...
    formStep && formStep.addEventListener("change", (e)=>{
        let missingKey = false;
        const formData = new FormData(formStep);
        const jsonData = {};
        formData.forEach((value, key)=>{
            jsonData[key] = value;
            if (!value || value == "") missingKey = true;
        });
        if (!missingKey && Object.keys(jsonData).length == 10) nextButton.classList.remove("disabled");
        else nextButton.classList.add("disabled");
    });
    // Formulaire 1: Informations avec étape
    formStep && formStep.addEventListener("submit", function(e) {
        e.preventDefault();
        if (isLoad) return;
        isLoading();
        const formData = new FormData(formStep);
        ...
        const jsonData = {};
        formData.forEach((value, key)=>{
            if (key == "phone") jsonData[key] = value.replace("0", "+33");
        ...
        fetch("/wp-json/contact-survey/v1/validate-contact", {
            ...
            isNotLoading();
            if (data.success) {
                if (data.ID) window.location.href = "?id=" + data.ID;
            } else if (data.message) console.log("\u274C Une erreur est survenue: ", data);
            else console.log("\u274C Erreur lors de la soumission du formulaire.");
        });
    });
    // Formulaire 2: Validation SMS
    formValidationStep && formValidationStep.addEventListener("submit", function(e) {
        e.preventDefault();
        if (isLoad) return;
        isLoading();
    ...
    resendSMSBtn && resendSMSBtn.addEventListener("click", function(e) {
        e.preventDefault();
        if (isLoad) return;
        isLoading();
        // Envoyez les données du formulaire à votre API
    ...
    formPreferencesStep && formPreferencesStep.addEventListener("submit", function(e) {
        e.preventDefault();
        if (isLoad) return;
        isLoading();
    ...
    ignore && ignore.addEventListener("click", ()=>{
        if (isLoad) return;
        isLoading();
        onSubmitPref(JSON.stringify({
...
},{}]},["2oeXr","le1MK"], "le1MK", "parcelRequire2151")

//# sourceMappingURL=contact-survey.js.map
